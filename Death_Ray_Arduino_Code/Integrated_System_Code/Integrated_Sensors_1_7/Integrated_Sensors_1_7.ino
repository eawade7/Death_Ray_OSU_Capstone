...
///////////////////// BEGIN PROGRAM LOOP //////////////////////////////////////////////////////
void loop() {
  float minTiltAngle = 40;
  float maxTiltAngle = 70;
  double minPanAngle = 90;
  double loadPanAngle = 180;
  ...
  float ampState = float(analogRead(PIN_CURRENT))/512.0*30.0-15.0;
  //Serial.println(ampState);
  ...
  if ((azimuth < loadPanAngle) && (azimuth > minPanAngle)){
    ...
    if ((encDegrees - azimuth) < -angleTolerance) {
      while (encDegrees < azimuth){
        turnCW();
        checkEncoder();
        Serial.print("Encodercw: ");
        Serial.println(encDegrees);
  ...
  brakePanMotor(); 
  //accel, encoder, current, elevation, azimuth
  printdata();
...
void goHome(){
  ...
  float ampState = float(analogRead(PIN_CURRENT))/512.0*30.0-15.0;
  while(ampState < 1){
    ampState = float(analogRead(PIN_CURRENT))/512.0*30.0-15.0;
...
void checkEncoder(){
  ...
  if (encDegrees < (oldDeg-50.0)){
    rotationCount++;
    encDegrees = encDegrees + float(rotationCount)*(360.0*(8.0/35.0));
  }
